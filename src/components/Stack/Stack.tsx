...
import { stackItems, type StackCategory } from './stackData';
        ...
        <section className="w-full">
            <div className="mx-auto md:max-w-4xl max-w-7xl">
                <div className="flex">
                    ...
                    <div className={`flex-1 px-4 py-6 space-y-6 ${isDark ? 'bg-[hsla(0,1%,14%,0.4)]' : 'bg-[hsla(0,0%,100%,0.4)]'}`}>
                        ...
                        <div className="flex justify-center gap-2">
                            {stackButtonVariants.map(({ label, category }) => (
                                <motion.button
                                    ...
                                    whileTap={{ scale: 0.95 }}
                                    className={`px-4 py-2 rounded-lg text-sm font-medium ${isDark ? '' : 'border border-gray-400'} transition-colors ${activeCategory === category
                                ...
                                {filteredStack.map((item) => (
                                    ...
                                    >
                                        <motion.img
                                            src={item.icon}
                                            alt={item.name}
                                            className={`w-10 h-10 md:w-10 md:h-10 object-contain ${isDark ? item.name === 'Next.js' ? 'invert brightness-0' : '' : ''}`}
                                            whileHover={{
                                                rotateY: 360,
                                                scale: 1.1,
                                            }}
